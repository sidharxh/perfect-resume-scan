...
export default function UploadArea({ id, onScanComplete }: UploadAreaProps) {
  ...
  const handleFileUpload = async (file: File) => {
    const startTime = Date.now(); // Client-side timer backup
    ...
    try {
      ...
      const data: any = await response.json(); // Using any temporarily to access meta fields freely
      ...
      if (data.output && data.analytics) {
        sendGAEvent('event', 'ats_scan_complete', {
          // Status & Type
          status: 'success',
          file_type: data.analytics.file_type || 'unknown',

          // Performance Metrics (Safe Fallbacks)
          duration_sec: data.analytics.processing_time || 0,
          tokens_input: data.analytics.tokens?.input || 0,
          tokens_output: data.analytics.tokens?.output || 0,
          tokens_total: data.analytics.tokens?.total || 0,

          // Business Metrics (Safe Fallbacks)
          ats_score: data.output.score || 0,
          score_label: sanitize(data.output.scoreLabel || 'unknown'), // Sanitize handles undefined

          // Improvement Insights (Safe Fallbacks)
          missing_keywords: data.analytics.missing_keywords_count || 0,
          improvement_count: data.analytics.improvements_count || 0,
          improvement_types: sanitize(data.analytics.improvement_types || ''), // Ensure string
      ...
      sendGAEvent('event', 'ats_scan_error', {
        status: 'error',
        error_message: sanitize(err.message),
        duration_sec: parseFloat(clientDuration)
      });
